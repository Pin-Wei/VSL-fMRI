...
PATH = 'logs//PW{:03d}//'.format(Subj_ID)
...
try:
    ...
    # Generate the shape stream
        ...
        labels = sc_labels
        ISI_list = list(pd.read_csv('sc_ISI_unshuffled.csv', header=None, squeeze=True))
...
## Experiment END ==============================================================
    ...
    EventOnset.to_csv(PATH +'sub-{:02d}_task-vsl_run-{:02d}_events.tsv'.format(Subj_ID, R), sep='\t', index=False)
    ...
    with open (PATH +'{:03d}_runTime.txt'.format(Subj_ID), MODE) as fn3:
        ...
        fn3.writelines('\nDuration from first to last stimuli event (+1s) = '+ str(round(toc-Trig_Time[0], 2)) +' (s)')
